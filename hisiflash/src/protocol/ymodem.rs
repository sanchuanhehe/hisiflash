        ...
        while start.elapsed() < timeout {
            ...
            match self.read_input(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    trace!("Ignoring non-control YMODEM response bytes: {:02X?}", chunk);
                },
                ...
                .c_timeout
        {
            ...
            match self.read_input(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    trace!("Ignoring bytes while waiting for 'C': {:02X?}", chunk);
                },
                Err(e) if e.kind() == std::io::ErrorKind::TimedOut => {},
                Err(e) => return Err(Error::Io(e)),
            }
                    ...
                    .char_timeout
            {
                ...
                match self.read_input(&mut buf) {
                    ...
                    Ok(n) => {
                        ...
                        transition_data.extend_from_slice(chunk);
                        trace!(
                            "Ignoring non-control YMODEM response bytes after finish block: {:02X?}",
                            chunk
                        );
                    },
                    Err(e) if e.kind() == std::io::ErrorKind::TimedOut => {},
            ...
            self.send_block(&block)
                .map_err(|err| {
                    Self::add_transfer_context(
                        err,
                        format!(
                            "while sending YMODEM data block {} for {} at offset 0x{offset:08X}",
                            seq, filename
                        ),
                    )
                })?;
